...
const PSF_PARAMS = {pi_c:18.8,pi_p:47.0,w_c:94.0,kappa:0.64};
const PSF_DATA = [{pi:1.88,w:72.34},{pi:4.25,w:77.98},{pi:6.62,w:82.78},{pi:8.99,w:86.72},{pi:11.36,w:89.81},{pi:13.72,w:92.05},{pi:16.09,w:93.44},{pi:18.46,w:93.99},{pi:20.83,w:93.69},{pi:23.2,w:92.54},{pi:25.57,w:90.53},{pi:27.94,w:87.68},{pi:30.31,w:83.98},{pi:32.67,w:79.45},{pi:35.04,w:74.05},{pi:37.41,w:67.8},{pi:39.78,w:60.7},{pi:42.15,w:52.75},{pi:44.52,w:43.96},{pi:46.89,w:34.31},{pi:49.26,w:23.81},{pi:51.62,w:12.51},{pi:53.99,w:0.32},{pi:56.36,w:-12.72},{pi:58.73,w:-26.62},{pi:61.1,w:-41.36}];

const MC_PARAMS = {n_draws:100000,mean:4.7,ci_lo:3.29,ci_hi:8.46,pct_hw:82.9,channels:[{name:"Primary welfare channel",dist:"LogNormal",lo:1.41,hi:3.29},{name:"Secondary externality",dist:"Triangular",lo:0.47,hi:1.88},{name:"Governance / institutional",dist:"Uniform",lo:0.24,hi:0.94}]};
const MC_DATA = [{bin:"0.3",count:223},{bin:"0.8",count:532},{bin:"1.3",count:1142},{bin:"1.8",count:2206},{bin:"2.3",count:3833},{bin:"2.8",count:5993},{bin:"3.3",count:8430},{bin:"3.8",count:10670},{bin:"4.3",count:12152},{bin:"4.8",count:12452},{bin:"5.3",count:11480},{bin:"5.9",count:9523},{bin:"6.4",count:7108},{bin:"6.9",count:4773},{bin:"7.4",count:2884},{bin:"7.9",count:1568},{bin:"8.4",count:767},{bin:"8.9",count:337},{bin:"9.4",count:133},{bin:"9.9",count:47}];

const THRESHOLDS = [{domain:"The Abyssal Recovery Floor — regulatory trigger",year:2026,status:"Institutional response threshold approaching",confidence:"Medium",crossed:true},{domain:"The Abyssal Recovery Floor — welfare crossover",year:2029,status:"Projected welfare-cost crossover point",confidence:"High",crossed:false},{domain:"The Abyssal Recovery Floor — systemic failure",year:2033,status:"System-level failure if unaddressed",confidence:"Medium-Low",crossed:false},{domain:"The Abyssal Recovery Floor — irreversibility",year:2038,status:"Irreversible welfare destruction threshold",confidence:"Low",crossed:false}];

const AXIOMS = {type:"impossibility",items:[{id:"A1",name:"Welfare Invisibility",description:"System welfare W cannot be computed from bilateral payoffs in The Abyssal Recovery Floor. The welfare function is structurally outside the negotiation space."},{id:"A2",name:"Private Pareto Trap",description:"Every Private Pareto improvement in The Abyssal Recovery Floor is compatible with unbounded welfare destruction. Efficiency and welfare are decoupled."},{id:"A3",name:"Institutional Incompleteness",description:"No finite set of institutional rules can guarantee W > 0 for all feasible extraction paths in The Abyssal Recovery Floor."}]};

const METHODS_DATA = {
  welfare_function: "W(t) = W_0 - \u03A3_i \u03B2_W(i) \u00B7 \u03A0_i(t) + \u03B7 \u00B7 R(t), where R(t) represents remediation effort. For The Abyssal Recovery Floor, W_0 is calibrated to pre-extraction baseline welfare. The aggregate \u03B2_W = 4.7 implies $4.7 of welfare destruction per $1 of private payoff.",
  cooperative_baseline: "The cooperative extraction level \u03A0_C = $18.8B maximizes joint surplus W + \u03A0. Current extraction \u03A0_P = $47.0B exceeds this by $28.2B, representing the over-extraction gap. Shadow price \u03BC* = 0.2128 (21.3%) defines the efficient welfare price.",
  falsification: ["If \u03B2_W < 1.0 under any reasonable social cost methodology, the PST classification fails for The Abyssal Recovery Floor.","If welfare costs are fully internalized through existing market mechanisms (Coase conditions hold), the institutional failure claim is falsified.","If Monte Carlo robustness drops below 80% for \u03B2_W > 1.0, the point estimate is unreliable.","If cooperative baseline extraction \u03A0_C exceeds current \u03A0_P, over-extraction claim is reversed."],
  key_sources: ["Postnieks, E. (2026). System Asset Pricing Model: The Abyssal Recovery Floor. SAPM Working Paper.","Rennert, K. et al. (2022). Comprehensive evidence implies a higher social cost of CO\u2082. Nature 610.","Arrow, K. (1951). Social Choice and Individual Values. Impossibility theorem foundations.","Stiglitz, J. (2019). People, Power, and Profits. Institutional failure mechanisms.","Nordhaus, W. (2017). Revisiting the social cost of carbon. PNAS 114(7)."]
};
